...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      // Use the optimized extraction for both mobile and desktop
      console.log("Using optimized DeepSeek extraction");
      recipe = await extractRecipeWithDeepSeekOptimized(url, isMobile);
    } catch (deepseekError) {
      console.error("Optimized DeepSeek extraction failed:", deepseekError);
